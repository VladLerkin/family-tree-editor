...
class OpenAiClient : AiClient {
    ...
    override suspend fun transcribeAudio(audioData: ByteArray, config: AiConfig): String {
        ...
        // Если это CAF файл, конвертируем его в WAV для Whisper API
        val (finalData, contentType, fileName) = if (isCAF) {
            println("[DEBUG_LOG] OpenAiClient: Converting CAF to WAV format")
            val wavData = convertCafToWav(audioData)
            println("[DEBUG_LOG] OpenAiClient: Converted to WAV (size: ${wavData.size} bytes)")
            Triple(wavData, "audio/wav", "audio.wav")
        } else {
            Triple(audioData, "audio/m4a", "audio.m4a")
        }
        
        println("[DEBUG_LOG] OpenAiClient: Sending audio format: $contentType (size: ${finalData.size} bytes)")
        ...
        try {
            ...
            println("[DEBUG_LOG] OpenAiClient: Whisper API response status: ${response.status}")
            
            val responseText = response.bodyAsText()
            println("[DEBUG_LOG] OpenAiClient: Whisper API response body: $responseText")
            ...
            if (text == null) {
                println("[DEBUG_LOG] OpenAiClient: No 'text' field in response. Response keys: ${responseJson.keys}")
    ...
    private fun convertCafToWav(cafData: ByteArray): ByteArray {
        ...
        if (dataOffset == -1 || dataSize <= 0) {
            println("[DEBUG_LOG] OpenAiClient: Failed to find data chunk in CAF, using original data")
        ...
        println("[DEBUG_LOG] OpenAiClient: Extracted PCM data from CAF: offset=$dataOffset, size=${pcmData.size}")
